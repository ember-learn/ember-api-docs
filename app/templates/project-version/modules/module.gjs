...
<template>
  ...
  <article class="chapter">
    <div class="article-title-wrapper">
      ...
      {{else}}
        <h1 class="module-name">Package
          {{@controller.model.name}}
          {{#if @controller.model.access}}<span
              class="access"
            >{{@controller.model.access}}</span>{{/if}}
        </h1>
      {{/if}}
    </div>
    <hr />
      ...
      {{#if @controller.model.parent}}
        <div class="attribute">
          ...
          <span class="attribute-value"><LinkTo
              ...
              @models={{array
                @controller.model.projectVersion.compactVersion
                @controller.model.parent
              }}
            >{{@controller.model.parent}}</LinkTo></span>
        </div>
      {{/if}}
    </p>

    <p class="description"><MarkdownToHtml
        @markdown={{@controller.model.description}}
      /></p>
        ...
        <ul class="spec-property-list">
          {{#each @controller.classesAndNamespaces as |klass|}}
            {{#if
              (not
                (and
                  (eq @controller.model.name "ember-data") (eq klass "Ember")
                )
              )
            }}
              <li>
    ...
    {{#if
      (and
        @controller.functionHeadings (gt @controller.functionHeadings.length 0)
      )
    }}
