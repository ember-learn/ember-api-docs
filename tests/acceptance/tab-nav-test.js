...
module('Acceptance | tab navigation', function (hooks) {
  ...
  test('switching tabs', async function (assert) {
    ...
    assert.equal(
      currentURLNoParams(),
      '/ember/1.0/classes/Ember.Component/methods',
      'navigated to methods'
    );
    assert
      .dom(`.tabbed-layout__menu ${'[data-test-tab="methods"]'}`)
      .hasClass('tabbed-layout__menu__item_selected', 'methods tab selected');

    await click(`.tabbed-layout__menu ${'[data-test-tab="properties"]'}`);

    assert.equal(
      currentURLNoParams(),
      '/ember/1.0/classes/Ember.Component/properties',
      'navigated to properties'
    );
    assert
      .dom(`.tabbed-layout__menu ${'[data-test-tab="properties"]'}`)
      .hasClass(
    ...
    assert.equal(
      currentURLNoParams(),
      '/ember/1.0/classes/Ember.Component/events',
      'navigated to events'
    );
    assert
      .dom(`.tabbed-layout__menu ${'[data-test-tab="events"]'}`)
      .hasClass('tabbed-layout__menu__item_selected', 'events tab selected');
    ...
    assert
      .dom(`.tabbed-layout__menu ${'[data-test-tab="index"]'}`)
      .hasClass('tabbed-layout__menu__item_selected', 'index tab selected');
  });
});
